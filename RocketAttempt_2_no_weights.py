...
a trajectory with pre-optimised position, velocity and mass through the control of thrust."""                                     
########################################################################################################
...
def create_next_generation(pop, pop_num, fit_val, mut_prob, kd_min, kd_max, kp_min, kp_max, ki_min, ki_max):
    ...
    for s in range(pop_num - len(pop_new)):
        ...
        #Into 2-D List. Access via pop[i][j]
        pop_new.append([kp_r, kp_v, kp_m, ki_r, ki_v, ki_m, kd_r, kd_v, kd_m])
    return pop_new
...
def fitness(pop):
    ...
    for s in range(len(pop)):
        ...
        print(" Currently at pop ",s, " / ", pop_num)
        r_error = 0.0
        v_error = 0.0
        m_error = 0.0
        for i in range(len(tref)):
...
while iteration < iteration_max:
    ...
    if iteration < iteration_max and iteration > 0:
        ...
        print(" Top population member: ", pop[0])
        print(" Fitness Value of pop[0]: ", fit_val[0])
